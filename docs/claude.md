...
```sh
npx devflow-agent-cli init --adapter claude
```
...
1. Run `npx devflow-agent-cli init --adapter claude`
...
```sh
npx devflow-agent-cli init   # picks claude if .claude/ exists
```

## Checklist

- [ ] Run `npx devflow-agent-cli init --adapter claude` in the repository root
...
- If you want Claude Code explicitly, run `npx devflow-agent-cli init --adapter claude`
...
```sh
npx devflow-agent-cli init --adapter claude --dry-run
npx devflow-agent-cli init --adapter claude --merge
npx devflow-agent-cli init --adapter claude --force
```
